...
def encode_activation(x, precision):
    if precision == "a32":
        return x, None
    if precision == "a16":
...
def activation_description(precision):
    return {
        "name": "W3" + precision.upper(),
        "quantized_locations": "normalized pooled input and post-ReLU hidden activation, immediately before each linear layer",
        ...
        "relu_note": "A3 hidden activations use only codes 0 and +1 because ReLU is nonnegative" if precision == "a3" else None,
    }
