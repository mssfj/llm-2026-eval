...
@torch.no_grad()
def candidate_losses(model, x, y, plan):
    ...
    if first==0:h=linear(None,0);start=0
        ...
        return _losses(logits,labels),dict(first=first,matrices=len(shapes),cache_bytes=sum(t.numel()*t.element_size() for t in inputs+outputs+streams))
    else:
