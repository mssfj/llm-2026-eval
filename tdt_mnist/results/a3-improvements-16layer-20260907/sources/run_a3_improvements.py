"""Execute the agreed nine 16-layer / threshold-8 runs, then analyze against 3 saved controls."""
...
BASE=ROOT/'tdt_mnist/runs/depth-activation-100k-20260907/identity-a3-threshold'
CONDITIONS=['rmsnorm','residual','lloyd']
def main():
    p=argparse.ArgumentParser();p.add_argument('--smoke',action='store_true');a=p.parse_args()
    out=Path('/tmp/tdt-a3-improvements-smoke-runs') if a.smoke else R
    report=Path('/tmp/tdt-a3-improvements-smoke-report') if a.smoke else P
    ...
    sources=['train.py','activation_quantization.py','depth_diagnostics.py','a3_improvements.py','run_a3_improvements.py']
    ...
    def run(condition,seed):
        ...
        opts={n:cfg.get(n) for n in names};opts.update(a3_improvement=condition,output_dir=str(run_dir),download=False,loss_diagnostics=True,layer_diagnostics=True)
    ...
    if not a.smoke:
        subprocess.run([sys.executable,str(ROOT/'tdt_mnist/analyze_a3_improvements.py'),str(report)],check=True)
