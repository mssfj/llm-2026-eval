"""100k activation depth x threshold sweep, with mandatory layer diagnostics."""
...
def report(root, records):
    ...
    lines=['# TDT 100k: 深さ × カウンタ閾値','',
        f'{records[0]["hidden_activation"]} / {records[0]["activation_precision"]} / {records[0]["a3_method"]}。層数は出力層を含む線形層の数。隠れ層の後だけにReLUを置く。',
...
def main():
    ...
    p.set_defaults(pool_shape=[9,10],hidden_sizes=DEPTH_WIDTHS[4],hidden_size=0,expected_params=100000,
        ...
        layer_diagnostics=True,loss_diagnostics=True,data_dir=Path('/tmp/tdt-mnist-data'),output_dir=Path('tdt_mnist/runs/depth-100k-20260907'))
    ...
    if (args.pool_shape!=[9,10] or args.expected_params!=100000 or args.hidden_size or args.activation_precision not in ('a32','a3')
        or not args.loss_diagnostics or not args.layer_diagnostics or args.oracle_every or args.test_size or args.data_seed is None):
    ...
    files=[Path(__file__).with_name(n).resolve() for n in ('train.py','activation_quantization.py','depth_diagnostics.py','sweep_depth_activation.py')]
