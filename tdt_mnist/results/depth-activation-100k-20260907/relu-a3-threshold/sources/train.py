...
class TernaryModel:
    ...
    def __init__(self, pool_size=10, hidden_size=0, zero_rate=1 / 3,
        ...
        if a3_method not in ("absmax", "mean_threshold") or not math.isfinite(a3_threshold_factor) or a3_threshold_factor <= 0:
            raise ValueError("invalid A3 quantizer settings")
...
def epoch(model, x, y, args, generator, scale, batch_generator=None):
    ...
    stats = {"fires": fires, "clip_rate": clipped / votes,
             "zero_difference_count": sum(d == 0.0 for d in differences),
             "zero_difference_fraction": sum(d == 0.0 for d in differences) / args.measurements,
    ...
    if getattr(args, "loss_diagnostics", False):
        stats["abs_y_values"] = differences
...
def parser():
    ...
    p.add_argument("--activation-precision", type=str.lower, choices=PRECISIONS, default="a32",
                   help="linear input encoding; A3 means three values, not three bits; accumulation stays FP32")
    ...
    p.add_argument("--loss-diagnostics", action=argparse.BooleanOptionalAction, default=False)
...
def validate(args, p):
    ...
    if args.a3_method != "absmax" and args.activation_precision != "a3":
        p.error("mean_threshold requires activation-precision a3")
...
def main():
    ...
    config.update(num_params=model.num_params, shapes=model.shapes, layer_scales=model.scales,
                  ...
                  activation_precision=args.activation_precision, activation_quantization=activation_description(
                      args.activation_precision, args.a3_method, args.a3_threshold_factor, args.hidden_activation),
    ...
    fields = ["step", "elapsed_seconds", "val_loss", "val_accuracy", "fires", "total_fires",
              ...
              "train_forward_examples", "total_forward_calls", "total_forward_examples",
              "zero_difference_count", "zero_difference_fraction"] + COUNTER_FIELDS
    abs_y = None
    if args.loss_diagnostics:
        import numpy as np
        abs_y = np.lib.format.open_memmap(args.output_dir / "abs_y.npy", mode="w+", dtype="float32",
                                        shape=(args.steps, args.measurements))
        fields += ["abs_y_mean", "abs_y_rms", "abs_y_median", "abs_y_p90", "abs_y_max"]
    total_fires = 0
    zero_differences = 0
    ...
    with (args.output_dir / "metrics.csv").open("w", newline="") as handle:
        ...
        for step in range(1, args.steps + 1):
            ...
            if abs_y is not None:
                values = np.asarray(stats.pop("abs_y_values"), dtype=np.float64)
                abs_y[step - 1] = values
                stats.update(abs_y_mean=float(values.mean()), abs_y_rms=float(np.sqrt(np.mean(values**2))),
                             abs_y_median=float(np.median(values)), abs_y_p90=float(np.quantile(values, .9)),
                             abs_y_max=float(values.max()))
                if step % args.eval_every == 0 or step == args.steps:
                    abs_y.flush()
            zero_differences += stats["zero_difference_count"]
    ...
    summary = {"activation_precision": args.activation_precision,
               "hidden_activation": args.hidden_activation, "a3_method": args.a3_method,
               "a3_threshold_factor": args.a3_threshold_factor,
               "zero_difference_count": zero_differences,
               "candidate_pair_count": args.measurements * args.steps,
               "zero_difference_fraction": zero_differences / (args.measurements * args.steps),
               "initial_activation_statistics": initial_activation_stats,
               "final_activation_statistics": final_activation_stats,
    ...
    if abs_y is not None:
        abs_y.flush()
        values = np.asarray(abs_y, dtype=np.float64).reshape(-1)
        summary["abs_y_statistics"] = {"count": len(values), "mean": float(values.mean()),
