...
class TernaryModel:
    ...
    def __call__(self, x, weights=None):
        ...
        for index, (shape, scale) in enumerate(zip(self.shapes, self.scales)):
            ...
            codes, activation_scale = encode_activation(x, self.activation_precision)
...
def main():
    ...
    config.update(num_params=model.num_params, shapes=model.shapes, layer_scales=model.scales,
                  ...
                  activation_precision=args.activation_precision, activation_quantization=activation_description(args.activation_precision),
